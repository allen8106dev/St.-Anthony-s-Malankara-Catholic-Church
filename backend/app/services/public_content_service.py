...
from app.models.domain import Announcement, Event, EventStatus, GalleryAlbum, HeroImage, LiturgyCollection, PageContent, PublicationStatus, ServiceTime, ServiceTimeStatus, SiteSetting
...
def liturgy(db: Session): return db.scalars(select(LiturgyCollection).options(selectinload(LiturgyCollection.resources)).where(LiturgyCollection.status == PublicationStatus.PUBLISHED).order_by(LiturgyCollection.sort_order, LiturgyCollection.created_at)).all()
