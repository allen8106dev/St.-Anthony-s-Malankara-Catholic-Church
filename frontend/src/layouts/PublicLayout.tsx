...
const pageMeta: Record<string, { title: string; description: string }> = {
  '/': { title: "St. Anthony's Malankara Catholic Church", description: 'A welcoming online home for parish life, prayer, and community.' },
...
export function PublicLayout() { const location = useLocation(); useEffect(() => { const meta = pageMeta[location.pathname] ?? { title: 'Page not found', description: 'The requested page is unavailable.' }; document.title = `${meta.title} | St. Anthony's Malankara Catholic Church`; document.querySelector('meta[name="description"]')?.setAttribute('content', meta.description) }, [location.pathname]); return <div className="site-shell"><PublicNavbar /><main><Outlet /></main><PublicFooter /><CustomCursor /></div> }
